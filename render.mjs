...
//   (fast previews without a GPU); --draft=0.25 paints at quarter resolution.
...
const gpu = process.platform === 'win32' ? ['--use-angle=d3d11'] : process.platform === 'darwin' ? ['--use-angle=metal'] : 
  existsSync('/dev/dri') ? ['--use-gl=angle'] : ['--use-angle=swiftshader', '--enable-unsafe-swiftshader'];   // no GPU: software WebGL
const browser = await puppeteer.launch({
  executablePath: CHROME, headless: true, protocolTimeout: 0,
  args: [...(process.getuid?.() === 0 ? ['--no-sandbox'] : []), '--allow-file-access-from-files', '--ignore-gpu-blocklist', ...gpu, '--enable-gpu-rasterization', '--window-size=1920,1080', '--disable-renderer-backgrounding', '--disable-background-timer-throttling']
});
